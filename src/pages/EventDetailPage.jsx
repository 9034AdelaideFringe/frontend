...
const EventDetailPage = () => {
  ...
  if (error) {
    return <div className={styles.errorContainer}>
      <h2>Error</h2>
      <p>{error}</p>
      <Link to="/events" className={styles.backButton}>Back to Events</Link>
    </div>;
  }
  ...
  if (!event) {
    return <div className={styles.errorContainer}>
      ...
      <p>Sorry, we couldn't find the event you're looking for. It may have been removed or the link is invalid.</p>
      <Link to="/events" className={styles.backButton}>Back to Events</Link>
    </div>;
  }
  ...
  const eventVenueLayout = event.venueseatinglayout || event.venueSeatingLayout || '';
  
  // Event description - retain all possible field names
  const eventDescription = event.short_description || event.description || event.des || 'No description available';
  
  // Format for display
  const displayDate = eventDate === 'Date TBA' ? eventDate : formatDate(eventDate);
  const displayTime = eventTime === 'Time TBA' ? eventTime : formatTime(eventTime);
  ...
  const priceDisplay = ticketTypes.length > 0
    ...
    : (event.price ? event.price : 'Price not specified');
    ...
    <div className={styles.eventDetailPage}>
      <div className={styles.eventHeader}>
        ...
        <div className={styles.eventMeta}>
          <div className={styles.metaItem}><i className="icon-calendar"></i> {displayDate}</div>
          <div className={styles.metaItem}><i className="icon-clock"></i> {displayTime}</div>
          <div className={styles.metaItem}><i className="icon-location"></i> {eventVenue || 'Location TBA'}</div>
          {eventCategory && <div className={styles.metaItem}><i className="icon-tag"></i> {eventCategory}</div>}
          {eventStatus && <div className={`${styles.statusTag} ${styles[eventStatus.toLowerCase()]}`}>{eventStatus}</div>}
        </div>
      </div>
      ...
      <div className={styles.eventContent}>
          ...
          {eventVenueLayout && (
            <div className={styles.venueLayout}>
              ...
              <img 
                ...
                onClick={() => window.open(getSeatLayoutUrl(eventVenueLayout), '_blank')}
                onError={(e) => { 
                  e.target.onerror = null; 
                  e.target.src = 'https://plus.unsplash.com/premium_photo-1724753996107-a35012f43bae?q=80&w=1470&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'; 
                }}
              />
            </div>
          )}
        ...
        <div className={styles.infoSection}>
            ...
            <div className={styles.detailsGrid}>
              ...
              {eventEndTime && (
                <div className={styles.detailItem}>
                  ...
                  <span className={styles.detailValue}>{formatDate(eventEndTime)}</span>
                </div>
              )}
              
              <div className={styles.detailItem}>
                <span className={styles.detailLabel}>Venue</span>
                <span className={styles.detailValue}>{eventVenue || 'TBA'}</span>
              </div>
              <div className={styles.detailItem}>
              ...
              {eventCapacity && (
                <div className={styles.detailItem}>
                  ...
                  <span className={styles.detailValue}>{eventCapacity} people</span>
                </div>
              )}
              {eventCategory && (
                <div className={styles.detailItem}>
            ...
            <div className={styles.ticketTypesSection}>
              ...
              <div className={styles.ticketTypesList}>
                {ticketTypes.map((ticket, index) => (
                  <div key={ticket.id || `ticket-${index}-${ticket.name}`} className={styles.ticketTypeCard}>
                    <div className={styles.ticketTypeHeader}>
                      <h3>{ticket.name}</h3>
                      <span className={styles.ticketPrice}>
                        {parseFloat(ticket.price) > 0 ? `$${parseFloat(ticket.price).toFixed(2)}` : 'Free'}
                      </span>
                    </div>
                    {ticket.description && (
                    ...
                    <div className={styles.ticketAvailability}>
                      <span>Available: </span>
                      <span className={parseInt(ticket.available_quantity || ticket.availableQuantity || 0) > 10 ? 
                        styles.available : styles.limited}>
                        {parseInt(ticket.available_quantity || ticket.availableQuantity || 0) > 0 ? 
                          (ticket.available_quantity || ticket.availableQuantity) : 'Sold Out'}
                      </span>
                    </div>
                  </div>
                ))}
          ...
          <div className={styles.actionSection}>
            ...
            >
              {eventStatus === 'ACTIVE' ? 'Buy Tickets' : 'Currently Unavailable'}
            </button>
            <Link to="/events" className={styles.backButton}>Back to Events</Link>
          </div>
        </div>
      ...
      {showTicketSelector && (
        <div className={styles.overlay}>
          <div className={styles.modalContent}>
            <button className={styles.closeButton} onClick={() => setShowTicketSelector(false)}>×</button>
