...
const EventDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        // Fetch event details
        console.log(
          "[EventDetailPage] Fetching event details with getEventById..."
        );
        const eventDataResponse = await getEventById(id);
        console.log(
          "[EventDetailPage] getEventById success:",
          eventDataResponse
        );
        ...
        if (Array.isArray(eventDataResponse)) {
          ...
          eventDataResponse &&
          (eventDataResponse.event_id || eventDataResponse.id)
        ) {
          eventData = eventDataResponse;
        } else {
          console.error(
            "[EventDetailPage] Invalid event data format received:",
            eventDataResponse
          );
        ...
        console.log(
          "[EventDetailPage] Fetching ticket types with getTicketTypesByEventIdAPI..."
        );
        const ticketTypesData = await getTicketTypesByEventIdAPI(id);
        console.log(
          "[EventDetailPage] getTicketTypesByEventIdAPI success:",
          ticketTypesData
        );
        ...
        if (Array.isArray(ticketTypesData)) {
          ...
          console.warn(
            "[EventDetailPage] getTicketTypesByEventIdAPI returned non-array data:",
            ticketTypesData
          );
          setTicketTypes([]);
        }
        ...
        console.log(
          "[EventDetailPage] Fetching occupied seats with getOccupiedSeatsByEventIdAPI..."
        );
        const occupiedSeatsData = await getOccupiedSeatsByEventIdAPI(id);
        console.log(
          "[EventDetailPage] getOccupiedSeatsByEventIdAPI success:",
          occupiedSeatsData
        );
        ...
        if (Array.isArray(occupiedSeatsData)) {
          ...
          console.warn(
            "[EventDetailPage] getOccupiedSeatsByEventIdAPI returned non-array data:",
            occupiedSeatsData
          );
        ...
        setError(
          err.message || "Failed to load event details or ticket types."
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleAddToCart = async (itemsToAdd) => {
    console.log(
      "[EventDetailPage] handleAddToCart called with items:",
      itemsToAdd
    );
    ...
    try {
      ...
      console.log(
        "[EventDetailPage] Successfully added to cart, re-fetching occupied seats..."
      );
      const updatedOccupiedSeats = await getOccupiedSeatsByEventIdAPI(id);
      setOccupiedSeats(updatedOccupiedSeats);
      console.log(
        "[EventDetailPage] Re-fetched occupied seats:",
    ...
    <div className={styles.eventDetailPage}>
      <div className={styles.eventHeader}>
        ...
        <div className={styles.eventMeta}>
          ...
          <div className={styles.metaItem}>
            <i className="icon-location"></i> Venue:{" "}
            {eventVenue || "Location TBA"}
          </div>
          <div className={styles.metaItem}>
      ...
      {eventStatus === "ACTIVE" && eventCategory && ticketTypes.length > 0 && (
        <>
          ...
          {console.log(
            "[EventDetailPage] Rendering SeatingLayoutSelector..."
          )}{" "}
      ...
      {eventStatus === "ACTIVE" &&
        (!eventCategory || ticketTypes.length === 0) && (
          <div className={styles.unavailableMessage}>
            Seating information or ticket types are not available for this
            event.
          </div>
        )}
    </div>
